...
class Translator(object):
    def __init__(self, opt):
        ...
        # print(nmt)
        embeddings = nmt.utils.createEmbeddings(model_opt, checkpoint['dicts'])

        model = nmt.utils.createNMT(model_opt, checkpoint['dicts'], embeddings)
    ...
    def buildData(self, srcBatch, goldBatch):
        ...
        if goldBatch:
            tgtData = [self.tgt_dict.convertToIdx(b,
                       ...
                       nmt.Constants.EOS_WORD) for b in goldBatch]

        return nmt.Dataset(srcData, tgtData, self.opt.batch_size,
                            self.opt.cuda, volatile=True,
                            data_type=self._type, balance=False)

    def buildTargetTokens(self, pred, src, attn):
        tokens = self.tgt_dict.convertToLabels(pred, nmt.Constants.EOS)
        tokens = tokens[:-1]  # EOS
        if self.opt.replace_unk:
            for i in range(len(tokens)):
                if tokens[i] == nmt.Constants.UNK_WORD:
    ...
    def translateBatch(self, srcBatch, tgtBatch):
        ...
        padMask = None
        if useMasking:
            padMask = srcBatch.data.eq(nmt.Constants.PAD).t()
        ...
        if tgtBatch is not None:
            decStates = None # initialize as None 
            # decOut = self.model.make_init_decoder_output(context)
            # mask(padMask)
            # initOutput = self.model.make_init_decoder_output(context)
            decOut, decStates, attn = self.model.decoder(
                tgtBatch[:-1], decStates, context)
            for dec_t, tgt_t in zip(decOut, tgtBatch[1:].data):
                gen_t = self.model.generator.forward(dec_t)
                tgt_t = tgt_t.unsqueeze(1)
                scores = gen_t.data.gather(1, tgt_t)
                scores.masked_fill_(tgt_t.eq(nmt.Constants.PAD), 0)
                goldScores += scores
        ...
        beam = [nmt.Beam(beamSize, self.opt.cuda) for k in range(batchSize)]
        ...
        for i in range(self.opt.max_sent_length):
            ...
            decOut, decStates, attn = self.model.decoder(
                Variable(input, volatile=True), decStates, context)
            ...
            for b in range(batchSize):
                ...
                for decState in decStates:  # iterate over h, c
					...
					nlayers = len(decStates)
					
					for i in range(nlayers):
						sentStates = decState[i].view(-1, beamSize, 
        ...
        for b in range(batchSize):
            ...
            allHyp += [hyps]
            # if useMasking:
                # valid_attn = srcBatch.data[:, b].ne(nmt.Constants.PAD) \
                                                # .nonzero().squeeze(1)
